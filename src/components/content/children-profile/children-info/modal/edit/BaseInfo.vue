...
// ? store
import { GET_USER_ENTRY_TIME, GET_USER_BIRTH_DATE } from '@store/storeType'
// ? mixins
import classInfoMixins from '@mixins/Common/classMixins'
import nationMixin from '@mixins/Common/nationMixin'
...
export default {
  ...
  mixins: [classInfoMixins, nationMixin],
  ...
  watch: {
    ...
    entryTime(newValue) {
      this.$store.commit(GET_USER_ENTRY_TIME, newValue)
    },
    birthDate(newValue) {
      this.$store.commit(GET_USER_BIRTH_DATE, newValue)
    },
    nationArr(newValue) {
